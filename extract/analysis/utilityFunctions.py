...
######################################
# getLatestData
# Searches for data directories and returns the latest one and its timestamp
# - return: tuple of two strings - the path to the data directory and its timestamp
def getLatestData():
	rawpaths = Path(".")

	neuron_list = None
	for x in reversed(sorted(rawpaths.iterdir())): # reverse loop through elements in this directory
...
# - return: list of neurons forming a random pattern
...
######################################
# arrayToStr
# Converts a numpy array to a sting of the format "1, 2, 3, 4"
# based on answer by dwitvliet on StackOverflow (https://stackoverflow.com/a/25085806)
# - array: the numpay array to be converted
# - return: the string in the specific format
def arrayToStr(array):
	if type(array) is not np.ndarray:
		raise TypeError("Provided variable is not a numpy array.")
	try:
		return str(list(array[0]))[1:-1] # numpy 1D array
	except TypeError:
		return str(list(array))[1:-1] # numpy sequence
