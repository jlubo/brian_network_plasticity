...
# computeFiringRates
# Computes firing rates for all subpoulations and for the individual neurons in the excitatory population
# full_path: the path to the spike raster data
# Ne: the number of excitatory neurons
# Ni: the number of inhibitory neurons
# Na: the number neurons in the assembly
# p_r: the fraction of neurons in the assembly that receive recall stimulation
...
def computeFiringRates(full_path, Ne, Ni, Na, p_r, readout_time_0, readout_time_1, 
                       col_sep = "\t\t", time_window_size = 0.5, other_simulator = False):
...
def extractParamsQMI(sup_directory, full_path, fout, col_sep = '\t\t', Ne = 1600, Ni = 400, time_window_size = 0.5, ref_time = 11.0, ref_dist = None):
	MI = []
	...
	if other_simulator:
		...
		D_pc = config['neuron'].get('diffusivity_pc') # use getter because this value only exists in multi-compartment simulations
	...
	v_exc_individual, (v_exc, v_exc_err, v_as, v_as_err, v_ans, v_ans_err, v_ctrl, v_ctrl_err, v_inh, v_inh_err) \
	  = computeFiringRates(full_path, Ne, Ni, Na, p_r, readout_time_0, readout_time_1, col_sep, time_window_size, other_simulator)
	if ref_dist is not None:
		print(f"Using reference distribution:\n {ref_dist}\nwith:\n {v_exc_individual[1]}")
...
def extractRecursion(sup_directory, fout, col_sep = '\t\t', Ne = 1600, Ni = 400, time_window_size = 0.5, ref_time = 11.0, ref_dist = None):
	...
	for x in sorted(rawpaths.iterdir()): # loop through elements in this directory
		...
		if hasTimestamp(path_tail) \
		   and ("_spikes.txt" in path_tail or "_spike_raster.txt" in path_tail):
			...
			extractParamsQMI(sup_directory, full_path, fout, col_sep, Ne, Ni, time_window_size, ref_time, ref_dist)
			data_found = True

		elif x.is_dir(): # recurse into the next directory

			ret = extractRecursion(sup_directory + os.sep + path_tail, fout, col_sep, Ne, Ni, time_window_size, ref_time, ref_dist)
